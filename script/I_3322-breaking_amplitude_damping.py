...
with respect to an amplitude damping channel acting on one side of the entangled
system.
"""
...
def I_3322_noise_optimization(prep_nodes, meas_nodes, **opt_kwargs):
    def _optimization_fn(noise_args):
        noise_nodes = [
            QNopt.NoiseNode(
                [1], lambda settings, wires: qml.AmplitudeDamping(noise_args, wires=wires[0])
            ),
        ]
...
def save_optimization_results(ansatz_name, param_range, opt_dicts):
    ...
    filename = "script/data/I_3322-breaking_amplitude_damping/" + ansatz_name + "/" + datetime_ext
    ...
    plt.title(r"One-Sided Amplitude Damping in the $I_{3322}$ Scenario:\n" + ansatz_name)
...
if __name__ == "__main__":
    ...
    max_entangled_optimization = I_3322_noise_optimization(
        max_entangled_prep, meas_nodes, sample_width=5, step_size=0.15, num_steps=80, verbose=False
    )
    ...
    arb_optimization = I_3322_noise_optimization(
        arb_prep, meas_nodes, sample_width=5, step_size=0.25, num_steps=60, verbose=False
    )
